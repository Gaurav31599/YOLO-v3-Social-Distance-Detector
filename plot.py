...
def bird_eye_view(frame, dist_mat, bottom_points, scale_w, scale_h, risk_count):
    h = frame.shape[0]
    w = frame.shape[1]
    ...
    for i in range(len(dist_mat)):
        ...
        if dist_mat[i][2] == 0:
            ...
            blank_image = cv2.line(blank_image, (int(dist_mat[i][0][0] * scale_w), int(dist_mat[i][0][1] * scale_h)), (int(dist_mat[i][1][0] * scale_w), int(dist_mat[i][1][1]* scale_h)), red, 2)
    ...
    for i in range(len(dist_mat)):
        ...
        if dist_mat[i][2] == 1:
            ...
            blank_image = cv2.line(blank_image, (int(dist_mat[i][0][0] * scale_w), int(dist_mat[i][0][1] * scale_h)), (int(dist_mat[i][1][0] * scale_w), int(dist_mat[i][1][1]* scale_h)), yellow, 2)
...
def social_distancing_view(frame, dist_mat, boxes, risk_count):
